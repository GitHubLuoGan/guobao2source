...
public class CommonDataEye{
	...
	public static void sdkInit(){
		...
		Log.d("commonDataEye","appid------>"+ appid);
		Log.d("commonDataEye","channel----->" + channel);
		
		
		DCAgent.setDebugMode(true);
		DCAgent.setUploadInterval(40);
	...
	public static void onResume(){
	    ...
	    new Handler(Looper.getMainLooper()).post(new Runnable() {
			...
			public void run() {
				...
				try {
					//todo
					Log.d("commonDataEye","XGpush --> appid " + appid);
					Log.d("commonDataEye","XGpush --> Activity " + CommonBaseSdk.sActivity.getLocalClassName());
					
					
					DCPushPlugin.registerPushByDataEye(CommonBaseSdk.sActivity, appid, DCPushPlatfrom.DC_XINGE, new DCPushOperateCallback() {

						@Override
						public void onSuccess(String data) {
							
							Log.d(Constants.LogTag, "+++ register push sucess. token:" + data);
							Log.d("commonDataEye","xgpush success!! token: " + data);
						}

						@Override
						public void onFail(int errCode, String msg) {
							
							Log.d(Constants.LogTag, "+++ register push fail.  errCode:" + errCode + ",msg:" + msg);
							Log.d("commonDataEye","xgpush fail!! errCode: " + errCode+ ",msg:" + msg);
						}
					});
	    ...
	    Intent service = new Intent(CommonBaseSdk.sActivity, XGPushService.class);
		CommonBaseSdk.sActivity.startService(service);
		Log.d("commonDataEye","XGpush Finish!");
	}
	
	public static void onPause(){ 
		
		Log.d("commonDataEye", "onPause");
	...
	public static void gameLevelStart(JsonValue parms){
		...
		Log.d("commonDataEye","gameLevelStart --->" + parms.toString());
		...
		Log.d("commonDataEye","gameLevelStart --->levelName" + levelId);
	...
	public static void gameLevelComplete(JsonValue parms){
		...
		Log.d("commonDataEye","gameLevelComplete --->" + parms.toString());
		...
		Log.d("commonDataEye","gameLevelComplete --->levelName" + levelName);
		Log.d("commonDataEye","gameLevelComplete --->isPass" + String.valueOf(isPass));
	...
	public static void userPaySuccess(JsonValue parms){
		...
		Log.d("commonDataEye","userPaySuccess --->" + parms.toString());
		JsonObject _json = parms.asObject();
		
		int resultCode = CommonBaseSdk.GetJsonValInt(_json,"code",-1);
		
		Log.d("commonDataEye","userPaySuccess --->resultCode" + String.valueOf(resultCode));
		if(resultCode == 1){
			...
			Log.d("commonDataEye","userPaySuccess --->paydata----->" + paydata.asObject().toString());
			Log.d("commonDataEye","userPaySuccess --->goodsData----->" + goodsData.asObject().toString());
			...
			Log.d("commonDataEye","userPaySuccess --->goodTotal--->" +  String.valueOf(CommonBaseSdk.GetJsonValInt(goodsData,"total",100)));
			Log.d("commonDataEye","userPaySuccess --->orderId--->" +  orderId);
			Log.d("commonDataEye","userPaySuccess --->goodsName--->" +  goodsName);
			Log.d("commonDataEye","userPaySuccess --->payAmount--->" +  String.valueOf(payAmount));
			...
			Log.d("commonDataEye","userPaySuccess --->" +  "用户支付失败！");
		}
	...
	public static void gameCoinAdd(JsonValue parms){
			...
			Log.d("commonDataEye","gameCoinAdd --->" + parms.toString());
	...
	public static void gameCoinLost(JsonValue parms){
		...
		Log.d("commonDataEye","gameCoinLost --->" + parms.toString());
	...
	public static void commonEvent(JsonValue parms){
		...
		Log.d("commonDataEye","commonEvent --->" + parms.toString());
	...
	public static void itemBuy(JsonValue parms){
		...
		Log.d("commonDataEye","itemBuy --->" + parms.toString());
	...
	public static void itemConsume(JsonValue parms){
		...
		Log.d("commonDataEye","itemConsume --->" + parms.toString());
