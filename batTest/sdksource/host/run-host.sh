...
#================================================================#
	...
	if [ -f  external/manifest_declarations.xml ]; then
		awk 'FNR==NR{ _[++d]=$0; next } /EXTERNAL DECLARATIONS:/ { print; print ""; for ( i=1; i<=d; i++ ) { print _[i] } next } 1'  external/manifest_declarations.xml $out_dir/project/AndroidManifest.xml > /tmp/AndroidManifest.tmp && mv -f /tmp/AndroidManifest.tmp $out_dir/project/AndroidManifest.xml
	fi
	#leiyoujian  end
	#cdsc end
